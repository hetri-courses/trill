...
use trill_core::config::CONFIG_TOML_FILE;
...
use trill_core::config_loader::ConfigLayerStack;
...
use trill_core::config_loader::load_config_layers_state;
...
pub(crate) async fn build_config_state() -> Result<ConfigState> {
    // Load config through `trill-core` so we inherit the same layer ordering and semantics as the
    ...
    let cfg_path = trill_home.join(CONFIG_TOML_FILE);
...
fn enforce_trusted_constraints(
    layers: &trill_core::config_loader::ConfigLayerStack,
...
fn network_proxy_constraints_from_trusted_layers(
    ...
    for layer in layers.get_layers(
        trill_core::config_loader::ConfigLayerStackOrdering::LowestPrecedenceFirst,
        false,
    ) {
