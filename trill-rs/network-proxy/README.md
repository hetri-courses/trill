...
`codex-network-proxy` is Trill's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from Trill's merged `config.toml` (via `codex-core` config loading).
