...
This repository uses Bazel to build the Rust workspace under `trill-rs`.
...
- `rules_rs` imports third-party crates from `trill-rs/Cargo.toml` and
  `trill-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `trill-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
