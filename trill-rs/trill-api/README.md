...
Typed clients for Trill/OpenAI APIs built on top of the generic transport in `trill-client`.
...
- **Prompted endpoints (Chat + Responses)**
  - Input: a single `Prompt` plus endpoint-specific options.
    - `Prompt` (re-exported as `trill_api::Prompt`) carries:
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `trill_api::CompactionInput`):
    - `model: &str`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `codex-api` and `trill-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
