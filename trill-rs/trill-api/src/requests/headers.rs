...
pub(crate) fn subagent_header(source: &Option<SessionSource>) -> Option<String> {
    ...
    match sub {
        trill_protocol::protocol::SubAgentSource::Review => Some("review".to_string()),
        trill_protocol::protocol::SubAgentSource::Compact => Some("compact".to_string()),
        trill_protocol::protocol::SubAgentSource::ThreadSpawn { .. } => {
            Some("collab_spawn".to_string())
        }
        trill_protocol::protocol::SubAgentSource::Other(label) => Some(label.clone()),
    }
}
