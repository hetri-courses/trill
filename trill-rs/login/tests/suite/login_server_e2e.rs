...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = trill_home.clone();

    let opts = ServerOptions {
        trill_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: trill_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = trill_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_trill_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let trill_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_trill_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        trill_home: first_trill_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: trill_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_trill_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        trill_home: second_trill_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: trill_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
