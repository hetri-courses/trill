...
- From the Trill CLI, run `trill execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
trill execpolicy check --rules path/to/policy.rules git status
```
