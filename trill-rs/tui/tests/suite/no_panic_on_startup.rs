...
/// Regression test for https://github.com/hetri-courses/trill/issues/8803.
#[tokio::test]
async fn malformed_rules_should_not_panic() -> anyhow::Result<()> {
    // run_trill_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
...
"#,
    ...
    std::fs::write(trill_home.join("config.toml"), config_contents)?;

    let CodexCliOutput { exit_code, output } = run_trill_cli(trill_home, cwd).await?;
    assert_ne!(0, exit_code, "Trill CLI should exit nonzero.");
    assert!(
...
async fn run_trill_cli(
    trill_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
    ...
    env.insert(
        ...
        trill_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = trill_utils_pty::spawn_pty_process(
        trill_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            ...
            anyhow::bail!("timed out waiting for trill CLI to exit");
        }
    };
