...
const BASE_BUG_ISSUE_URL: &str =
    "https://github.com/hetri-courses/trill/issues/new?template=2-bug-report.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const CODEX_FEEDBACK_INTERNAL_URL: &str = "http://go/trill-feedback-internal";
...
pub(crate) struct FeedbackNoteView {
    ...
    snapshot: trill_feedback::CodexLogSnapshot,
            ...
            Ok(()) => {
                ...
                let mut lines = vec![Line::from(match issue_url.as_ref() {
                    Some(_) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        format!("{prefix} Please report this in #trill-feedback:")
                    }
                ...
                match issue_url {
                    Some(url) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        lines.extend([
                            ...
                            Line::from(vec![
                                "    ".into(),
                                format!("go/trill-feedback/{thread_id}").bold(),
                            ]),
                        ]);
                    }
                    Some(url) => {
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "trill-logs.log".into()]).into(),
    ];
...
mod tests {
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = trill_feedback::CodexFeedback::new().snapshot(None);
    ...
    fn issue_url_available_for_bug_bad_result_and_other() {
        ...
        let expected_slack_url = "http://go/trill-feedback-internal".to_string();
