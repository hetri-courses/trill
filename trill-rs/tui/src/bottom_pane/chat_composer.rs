...
//!
//! For the detailed burst state machine, see `trill-rs/tui/src/bottom_pane/paste_burst.rs`.
...
use trill_common::fuzzy_match::fuzzy_match;
use trill_protocol::custom_prompts::CustomPrompt;
use trill_protocol::custom_prompts::PROMPTS_CMD_PREFIX;
...
use trill_chatgpt::connectors::AppInfo;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let slug = trill_core::connectors::connector_mention_slug(connector);
...
程序员 与 Unicode 同行";
    ...
    fn pasting_filepath_attaches_image() {
        ...
        let tmp_path: PathBuf = tmp.path().join("trill_tui_test_paste_image.png");
