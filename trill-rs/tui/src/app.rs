...
use trill_ansi_escape::ansi_escape_line;
use trill_app_server_protocol::ConfigLayerSource;
...
use trill_core::config_loader::ConfigLayerStackOrdering;
use trill_core::features::Feature;
use trill_core::models_manager::manager::RefreshStrategy;
use trill_core::models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use trill_core::models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
use trill_core::protocol::FinalOutput;
...
use trill_core::protocol::SkillErrorInfo;
...
use trill_protocol::openai_models::ModelUpgrade;
use trill_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use trill_protocol::protocol::SessionConfiguredEvent;
...
fn session_summary(
    ...
    let resume_command = trill_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
impl App {
    ...
    async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        ConfigBuilder::default()
            .trill_home(self.config.trill_home.clone())
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let trill_op_tx = crate::chatwidget::spawn_op_forwarder(thread);
        self.chat_widget = ChatWidget::new_with_op_sender(init, trill_op_tx);
    ...
    pub async fn run(
        ...
        let otel_manager = OtelManager::new(
            ...
            trill_core::terminal::user_agent(),
            SessionSource::Cli,
        );
        ...
        #[cfg(target_os = "windows")]
        {
            let should_check = WindowsSandboxLevel::from_config(&app.config)
                ...
                && matches!(
                    ...
                    trill_core::protocol::SandboxPolicy::WorkspaceWrite { .. }
                        | trill_core::protocol::SandboxPolicy::ReadOnly
                )
                && !app
                    .config
                    .notices
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.trill_home.clone();
        ...
        match event {
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let trill_home = self.config.trill_home.clone();
                    let tx = self.app_event_tx.clone();

                    // If the elevated setup already ran on this machine, don't prompt for
                    // elevation again - just flip the config to use the elevated path.
                    if trill_core::windows_sandbox::sandbox_setup_is_complete(trill_home.as_path())
                    {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = trill_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                        ...
                        let event = match result {
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    trill_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let mut builder =
                        ConfigEditsBuilder::new(&self.config.trill_home).with_profile(profile);
                    if elevated_enabled {
            ...
            AppEvent::UpdateSandboxPolicy(policy) => {
                ...
                #[cfg(target_os = "windows")]
                if !matches!(&policy, trill_core::protocol::SandboxPolicy::ReadOnly)
                    || WindowsSandboxLevel::from_config(&self.config)
                        != WindowsSandboxLevel::Disabled
                {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.trill_home.clone();
            ...
            AppEvent::UpdateFeatureFlags { updates } => {
                ...
                let mut builder = ConfigEditsBuilder::new(&self.config.trill_home)
            ...
            AppEvent::PersistFullAccessWarningAcknowledged => {
                if let Err(err) = ConfigEditsBuilder::new(&self.config.trill_home)
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("trill-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> trill_core::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
    ...
    fn spawn_world_writable_scan(
        ...
        sandbox_policy: trill_core::protocol::SandboxPolicy,
        tx: AppEventSender,
    ) {
        tokio::task::spawn_blocking(move || {
            let result = trill_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
mod tests {
    ...
    fn all_model_presets() -> Vec<ModelPreset> {
        trill_core::models_manager::model_presets::all_model_presets().clone()
    }
    ...
    async fn model_migration_prompt_shows_for_hidden_model() {
        let trill_home = tempdir().expect("temp codex home");
