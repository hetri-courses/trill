...
use trill_core::git_info::resolve_root_git_project_for_trust;
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trust_level(&self.trill_home, &target, TrustLevel::Trusted) {
    ...
    fn handle_dont_trust(&mut self) {
        ...
        let target =
            resolve_root_git_project_for_trust(&self.cwd).unwrap_or_else(|| self.cwd.clone());
        if let Err(e) = set_project_trust_level(&self.trill_home, &target, TrustLevel::Untrusted) {
...
mod tests {
    ...
    fn release_event_does_not_change_selection() {
        let trill_home = TempDir::new().expect("temp home");
