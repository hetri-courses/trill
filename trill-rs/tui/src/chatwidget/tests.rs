...
//!
//! These tests treat the widget as the adapter between `trill_core::protocol::EventMsg` inputs and
...
use trill_core::config::ConfigBuilder;
use trill_core::config::Constrained;
use trill_core::config::ConstraintError;
use trill_core::config_loader::RequirementSource;
...
use trill_core::protocol::ExecPolicyAmendment;
...
use trill_core::protocol::TurnStartedEvent;
...
use trill_protocol::openai_models::ReasoningEffortPreset;
...
async fn test_config() -> Config {
    ...
    let trill_home = std::env::temp_dir();
    ConfigBuilder::default()
        .trill_home(trill_home.clone())
        .build()
        .await
        .expect("config")
}
...
async fn resumed_initial_messages_render_history() {
    ...
    let configured = trill_core::protocol::SessionConfiguredEvent {
...
async fn interrupted_turn_restores_queued_messages_with_images_and_elements() {
    ...
    chat.handle_codex_event(Event {
        id: "interrupt".into(),
        msg: EventMsg::TurnAborted(trill_core::protocol::TurnAbortedEvent {
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let init = ChatWidgetInit {
        ...
        feedback: trill_feedback::CodexFeedback::new(),
        is_first_run: true,
...
fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> = trill_core::parse_command::parse_command(&command);
...
async fn exec_end_without_begin_uses_event_command() {
    ...
    let parsed_cmd = trill_core::parse_command::parse_command(&command);
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    chat.handle_codex_event(Event {
...
async fn unified_exec_wait_after_final_agent_message_snapshot() {
    ...
    begin_unified_exec_startup(&mut chat, "call-wait", "proc-1", "cargo test -p trill-core");
...
async fn collaboration_modes_defaults_to_code_on_startup() {
    let trill_home = tempdir().expect("tempdir");
...
async fn set_model_updates_active_collaboration_mask() {
    ...
    chat.set_model("gpt-5.1-trill-mini");

    assert_eq!(chat.current_model(), "gpt-5.1-trill-mini");
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Plan);
}
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    let rollout_path = PathBuf::from("/tmp/trill-test-rollout.jsonl");
...
async fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        trill_core::git_info::CommitLogEntry {
...
async fn model_reasoning_selection_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.1-trill-max")).await;

    set_chatgpt_auth(&mut chat);
    chat.set_reasoning_effort(Some(ReasoningEffortConfig::High));

    let preset = get_available_model(&chat, "gpt-5.1-trill-max");
...
async fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CodexOp(Op::PatchApproval { id, decision }) = app_ev {
            ...
            assert_matches!(decision, trill_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto trill_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
