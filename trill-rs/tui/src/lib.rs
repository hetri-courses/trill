...
// The standalone `trill-tui` binary prints a short help message before the
...
use trill_cloud_requirements::cloud_requirements_loader;
use trill_common::oss::ensure_oss_provider_ready;
use trill_common::oss::get_default_model_for_oss_provider;
use trill_common::oss::ollama_chat_deprecation_notice;
use trill_core::AuthManager;
use trill_core::CodexAuth;
use trill_core::INTERACTIVE_SESSION_SOURCES;
use trill_core::RolloutRecorder;
use trill_core::ThreadSortKey;
use trill_core::auth::enforce_login_restrictions;
use trill_core::config::Config;
use trill_core::config::ConfigBuilder;
use trill_core::config::ConfigOverrides;
use trill_core::config::find_trill_home;
use trill_core::config::load_config_as_toml_with_cli_overrides;
use trill_core::config::resolve_oss_provider;
use trill_core::config_loader::CloudRequirementsLoader;
use trill_core::config_loader::ConfigLoadError;
use trill_core::config_loader::format_config_error_with_source;
use trill_core::default_client::set_default_client_residency_requirement;
use trill_core::find_thread_path_by_id_str;
use trill_core::find_thread_path_by_name_str;
use trill_core::path_utils;
use trill_core::protocol::AskForApproval;
use trill_core::read_session_meta_line;
use trill_core::terminal::Multiplexer;
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = trill_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let trill_home = match find_trill_home() {
        Ok(trill_home) => trill_home.to_path_buf(),
        Err(err) => {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&trill_home).await?;
    ...
    let log_dir = trill_core::config::log_dir(&config)?;
    ...
    // Trill CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("trill-tui.log"))?;
    ...
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("trill_core=info,trill_tui=info,trill_rmcp_client=info")
        })
    };
    ...
    let feedback = trill_feedback::CodexFeedback::new();
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        trill_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, true)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
            #[allow(clippy::print_stderr)]
    ...
    let log_db_layer = trill_core::state_db::get_state_db(&config, None)
        .await
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: trill_core::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let auth_manager = AuthManager::shared(
        initial_config.trill_home.clone(),
        false,
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let path = if is_uuid {
                find_thread_path_by_id_str(&config.trill_home, id_str).await?
            } else {
                find_thread_path_by_name_str(&config.trill_home, id_str).await?
            };
            match path {
...
fn determine_alt_screen_mode(no_alt_screen: bool, tui_alternate_screen: AltScreenMode) -> bool {
    if no_alt_screen {
        ...
        match tui_alternate_screen {
            ...
            AltScreenMode::Auto => {
                let terminal_info = trill_core::terminal::terminal_info();
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let trill_home = config.trill_home.clone();
        match CodexAuth::from_auth_storage(&trill_home, config.cli_auth_credentials_store_mode) {
...
mod tests {
    ...
    use trill_core::config::ConfigBuilder;
    use trill_core::config::ConfigOverrides;
    use trill_core::config::ProjectConfig;
    use trill_core::protocol::AskForApproval;
    use trill_protocol::protocol::RolloutItem;
    use trill_protocol::protocol::RolloutLine;
    use trill_protocol::protocol::SessionMeta;
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .trill_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let trill_home = temp_dir.path().to_path_buf();
