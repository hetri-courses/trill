...
use trill_app_server_protocol::CancelLoginChatGptParams;
use trill_app_server_protocol::ClientInfo;
use trill_app_server_protocol::ClientNotification;
...
use trill_app_server_protocol::InitializeParams;
...
use trill_app_server_protocol::JSONRPCMessage;
...
use trill_app_server_protocol::ServerRequest;
...
pub const DEFAULT_CLIENT_NAME: &str = "trill-app-server-tests";

impl McpProcess {
    pub async fn new(trill_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(trill_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = trill_utils_cargo_bin::cargo_bin("trill-app-server")
            .context("should find binary for trill-app-server")?;
        let mut cmd = Command::new(program);

        cmd.stdin(Stdio::piped());
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        cmd.env("CODEX_HOME", trill_home);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("trill-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
