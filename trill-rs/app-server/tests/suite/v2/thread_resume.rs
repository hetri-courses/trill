...
use trill_protocol::user_input::ByteRange;
...
async fn thread_resume_returns_original_thread() -> Result<()> {
    ...
    let start_id = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("gpt-5.1-trill-max".to_string()),
            ..Default::default()
        })
        .await?;
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let trill_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(trill_home.path(), &server.uri())?;
...
fn create_config_toml(trill_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
fn setup_rollout_fixture(trill_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(trill_home, server_uri)?;
    ...
    let rollout_file_path = rollout_path(trill_home, filename_ts, &conversation_id);
