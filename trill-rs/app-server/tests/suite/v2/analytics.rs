...
use trill_core::config::types::OtelExporterKind;
use trill_core::config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut trill_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = trill_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
