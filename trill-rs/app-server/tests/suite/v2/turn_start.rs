...
use trill_app_server_protocol::ByteRange;
...
use trill_app_server_protocol::PatchChangeKind;
use trill_app_server_protocol::RequestId;
use trill_app_server_protocol::ServerRequest;
use trill_app_server_protocol::TextElement;
...
use trill_core::protocol_config_types::ReasoningSummary;
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(trill_home.path()).await?;
    timeout(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    assert_eq!(
        started.turn.status,
        trill_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-trill-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    ...
    let image_path = trill_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let trill_home = tmp.path().to_path_buf();
    ...
    let mut mcp = McpProcess::new(trill_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": trill_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(trill_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(trill_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let trill_home = tmp.path().join("trill_home");
    std::fs::create_dir(&trill_home)?;
    ...
    let mut mcp = McpProcess::new(&trill_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(trill_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(trill_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![trill_app_server_protocol::FileUpdateChange {
...
fn create_config_toml(
    ...
    let feature_entries = features
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let config_toml = trill_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
