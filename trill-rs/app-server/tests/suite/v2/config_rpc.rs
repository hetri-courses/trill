...
use trill_app_server_protocol::ConfigEdit;
use trill_app_server_protocol::ConfigLayerSource;
use trill_app_server_protocol::ConfigReadParams;
use trill_app_server_protocol::ConfigReadResponse;
use trill_app_server_protocol::ConfigValueWriteParams;
use trill_app_server_protocol::ConfigWriteResponse;
use trill_app_server_protocol::JSONRPCError;
use trill_app_server_protocol::JSONRPCResponse;
use trill_app_server_protocol::MergeStrategy;
use trill_app_server_protocol::RequestId;
use trill_app_server_protocol::SandboxMode;
use trill_app_server_protocol::ToolsV2;
use trill_app_server_protocol::WriteStatus;
use trill_core::config::set_project_trust_level;
use trill_core::config_loader::SYSTEM_CONFIG_TOML_FILE_UNIX;
...
fn write_config(trill_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
        trill_home.path().join("config.toml"),
        contents,
    )?)
}
...
"#,
    ...
    let trill_home_path = trill_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(trill_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let trill_home = TempDir::new()?;
    write_config(&trill_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(trill_home.path(), workspace.path(), TrustLevel::Trusted)?;
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let trill_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", trill_home)?;
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(trill_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let trill_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(trill_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[trill_app_server_protocol::ConfigLayer],
