...
use crate::trill_tool_config::TrillToolCallParam;
use crate::trill_tool_config::TrillToolCallReplyParam;
use crate::trill_tool_config::create_tool_for_codex_tool_call_param;
use crate::trill_tool_config::create_tool_for_codex_tool_call_reply_param;
...
use trill_core::protocol::Submission;
...
impl MessageProcessor {
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "trill-mcp-server".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
                title: Some("Trill".to_string()),
    ...
    async fn handle_tool_call_codex(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<TrillToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg
                    .into_config(self.trill_linux_sandbox_exe.clone())
                    .await
                {
                    Ok(cfg) => cfg,
                    Err(e) => {
        ...
        task::spawn(async move {
            ...
            crate::trill_tool_runner::run_codex_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_codex_session_reply(
        ...
        // parse arguments
        let codex_tool_call_reply_param: TrillToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<TrillToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
            ...
            None => {
                tracing::error!(
                    "Missing arguments for trill-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_owned(),
                        text: "Missing arguments for trill-reply tool-call; the `thread_id` and `prompt` fields are required.".to_owned(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        let codex = match self.thread_manager.get_thread(thread_id).await {
            ...
            Err(_) => {
                ...
                let result = crate::trill_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        tokio::spawn({
            ...
            async move {
                crate::trill_tool_runner::run_codex_tool_session_reply(
                    thread_id,
                    codex,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(
        ...
        let err = codex_arc
            .submit_with_id(Submission {
                ...
                op: trill_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
