...
use crate::state_db;
use trill_file_search as file_search;
...
pub(crate) async fn get_threads(
    ...
    let root = trill_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.trill/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    let mut root = trill_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
    ...
    let state_db_ctx = state_db::open_if_present(trill_home, "").await;
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(trill_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(trill_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
