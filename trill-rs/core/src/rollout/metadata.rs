...
use trill_state::BackfillStats;
use trill_state::DB_ERROR_METRIC;
use trill_state::DB_METRIC_BACKFILL;
use trill_state::DB_METRIC_BACKFILL_DURATION_MS;
use trill_state::ExtractionOutcome;
use trill_state::ThreadMetadataBuilder;
use trill_state::apply_rollout_item;
use std::cmp::Reverse;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(
    runtime: &trill_state::StateRuntime,
    config: &Config,
    otel: Option<&OtelManager>,
) {
    let timer = otel.and_then(|otel| otel.start_timer(DB_METRIC_BACKFILL_DURATION_MS, &[]).ok());
    let sessions_root = config.trill_home.join(rollout::SESSIONS_SUBDIR);
    let archived_root = config.trill_home.join(rollout::ARCHIVED_SESSIONS_SUBDIR);
...
mod tests {
    ...
    use chrono::Utc;
    use trill_protocol::ThreadId;
    use trill_protocol::protocol::CompactedItem;
    use trill_protocol::protocol::RolloutItem;
