...
use trill_protocol::openai_models::ApplyPatchToolType;
...
use trill_protocol::openai_models::TruncationMode;
...
const GPT_5_1_CODEX_MAX_INSTRUCTIONS: &str = include_str!("../../gpt-5.1-trill-max_prompt.md");
...
pub(crate) fn find_model_info_for_slug(slug: &str) -> ModelInfo {
    ...
    } else if slug.starts_with("trill-mini-latest") {
        model_info!(
            slug,
            base_instructions: BASE_INSTRUCTIONS_WITH_APPLY_PATCH.to_string(),
    ...
    } else if slug.starts_with("exp-codex") || slug.starts_with("trill-1p") {
        model_info!(
            slug,
    ...
    } else if slug.starts_with("gpt-5.1-trill-max") {
        model_info!(
            slug,
