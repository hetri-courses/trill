...
use trill_api::AggregateStreamExt;
use trill_api::ChatClient as ApiChatClient;
use trill_api::CompactClient as ApiCompactClient;
use trill_api::CompactionInput as ApiCompactionInput;
use trill_api::Prompt as ApiPrompt;
use trill_api::RequestTelemetry;
use trill_api::ReqwestTransport;
use trill_api::ResponseAppendWsRequest;
use trill_api::ResponseCreateWsRequest;
use trill_api::ResponseStream as ApiResponseStream;
use trill_api::ResponsesClient as ApiResponsesClient;
use trill_api::ResponsesOptions as ApiResponsesOptions;
use trill_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use trill_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use trill_api::SseTelemetry;
use trill_api::TransportError;
use trill_api::build_conversation_headers;
use trill_api::common::Reasoning;
use trill_api::common::ResponsesWsRequest;
use trill_api::create_text_param_for_request;
use trill_api::error::ApiError;
use trill_api::requests::responses::Compression;
...
pub const X_CODEX_TURN_STATE_HEADER: &str = "x-trill-turn-state";
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-trill-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-trill-turn-state` request header for
...
impl ModelClientSession {
    ...
    async fn websocket_connection(
        &mut self,
        api_provider: trill_api::Provider,
    ...
    async fn stream_responses_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &*CODEX_RS_SSE_FIXTURE {
            ...
            let stream =
                trill_api::stream_from_fixture(path, self.state.provider.stream_idle_timeout())
                    .map_err(map_api_error)?;
...
/// Adapts the core `Prompt` type into the `trill-api` payload shape.
...
fn experimental_feature_headers(config: &Config) -> ApiHeaderMap {
    ...
    {
        headers.insert("x-trill-beta-features", header_value);
    }
    headers
}
