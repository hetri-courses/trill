...
pub enum Feature {
    ...
    PowershellUtf8,
    /// Compress request bodies (zstd) when sending streaming requests to trill-backend.
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://github.com/hetri-courses/trill/blob/main/docs/config.md#feature-flags for details."
                ))
            };
            (summary, details)
        }
