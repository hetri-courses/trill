...
use trill_protocol::protocol::ApplyPatchApprovalRequestEvent;
use trill_protocol::protocol::Event;
use trill_protocol::protocol::EventMsg;
use trill_protocol::protocol::ExecApprovalRequestEvent;
use trill_protocol::protocol::Op;
use trill_protocol::protocol::RequestUserInputEvent;
use trill_protocol::protocol::SessionSource;
use trill_protocol::protocol::SubAgentSource;
use trill_protocol::protocol::Submission;
...
use crate::trill::SUBMISSION_CHANNEL_CAPACITY;
...
pub(crate) async fn run_trill_thread_interactive(
    config: Config,
...
pub(crate) async fn run_trill_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = run_trill_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> trill_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = trill_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, trill_protocol::protocol::ReviewDecision::Abort)
                .await;
            trill_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
mod tests {
    ...
    use trill_protocol::protocol::TurnAbortedEvent;
    ...
    async fn forward_events_cancelled_while_send_blocked_shuts_down_delegate() {
        ...
        let (session, ctx, _rx_evt) = crate::trill::make_session_and_context_with_rx().await;
        let codex = Arc::new(Codex {
            next_id: AtomicU64::new(0),
            tx_sub,
            rx_event: rx_events,
