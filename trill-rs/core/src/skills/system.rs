...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".trill-system-skills.marker";
...
pub(crate) fn system_cache_root_dir(trill_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(trill_home)
        .and_then(|trill_home| system_cache_root_dir_abs(&trill_home))
...
fn system_cache_root_dir_abs(trill_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    trill_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}
...
/// install is skipped.
pub(crate) fn install_system_skills(trill_home: &Path) -> Result<(), SystemSkillsError> {
    let trill_home = AbsolutePathBuf::try_from(trill_home)
        .map_err(|source| SystemSkillsError::io("normalize codex home dir", source))?;
    let skills_root_dir = trill_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&trill_home)
