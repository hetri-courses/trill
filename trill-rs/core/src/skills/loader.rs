...
use trill_protocol::protocol::SkillScope;
...
mod tests {
    ...
    const REPO_ROOT_CONFIG_DIR_NAME: &str = ".trill";

    async fn make_config(trill_home: &TempDir) -> Config {
        make_config_for_cwd(trill_home, trill_home.path().to_path_buf()).await
    }

    async fn make_config_for_cwd(trill_home: &TempDir, cwd: PathBuf) -> Config {
        let trust_root = cwd
            .ancestors()
    ...
    fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
        ...
        let dot_codex = project_root.join(".trill");
    ...
    fn write_skill(trill_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
        write_skill_at(&trill_home.path().join("skills"), dir, name, description)
    }
    ...
    fn write_system_skill(
        ...
        write_skill_at(
            &trill_home.path().join("skills/.system"),
            dir,
            name,
            description,
        )
    ...
    async fn loads_skill_dependencies_metadata_from_yaml() {
        let trill_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&trill_home, "demo", "dep-skill", "from json");
...
"#,
        ...
        let cfg = make_config(&trill_home).await;
    ...
    async fn loads_skill_interface_metadata_from_yaml() {
        let trill_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&trill_home, "demo", "ui-skill", "from json");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn loads_skills_via_symlinked_subdir_for_user_scope() {
        ...
        fs::create_dir_all(trill_home.path().join("skills")).unwrap();
        symlink_dir(shared.path(), &trill_home.path().join("skills/shared"));
    ...
    async fn does_not_loop_on_symlink_cycle_for_user_scope() {
        ...
        let cycle_dir = trill_home.path().join("skills/cycle");
    ...
    async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
        ...
        let cfg = make_config_for_cwd(&trill_home, repo_dir.path().to_path_buf()).await;
        let outcome = load_skills(&cfg);
    ...
    async fn system_scope_ignores_symlinked_subdir() {
        ...
        let system_root = trill_home.path().join("skills/.system");
    ...
    async fn loads_valid_skill() {
        let trill_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&trill_home, "demo", "demo-skill", "does things\ncarefully");
        let cfg = make_config(&trill_home).await;
    ...
    async fn skips_hidden_and_invalid() {
        let trill_home = tempfile::tempdir().expect("tempdir");
        let hidden_dir = trill_home.path().join("skills/.hidden");
        ...
        let invalid_dir = trill_home.path().join("skills/invalid");
    ...
    async fn enforces_length_limits() {
        ...
        write_skill(&trill_home, "max-len", "max-len", &max_desc);
        let cfg = make_config(&trill_home).await;
        ...
        write_skill(&trill_home, "too-long", "too-long", &too_long_desc);
        let outcome = load_skills(&cfg);
        assert_eq!(outcome.skills.len(), 1);
        assert_eq!(outcome.errors.len(), 1);
        assert!(
    ...
    async fn loads_skills_from_all_codex_dirs_under_project_root() {
        ...
        let cfg = make_config_for_cwd(&trill_home, nested_dir).await;
    ...
    async fn loads_skills_from_codex_dir_when_not_git_repo() {
        ...
        let cfg = make_config_for_cwd(&trill_home, work_dir.path().to_path_buf()).await;
    ...
    async fn keeps_duplicate_names_from_repo_and_user() {
        ...
        let user_skill_path = write_skill(&trill_home, "user", "dupe-skill", "from user");
    ...
    async fn repo_skills_search_does_not_escape_repo_root() {
        ...
        let cfg = make_config_for_cwd(&trill_home, repo_dir).await;
    ...
    async fn loads_skills_when_cwd_is_file_in_repo() {
        ...
        let cfg = make_config_for_cwd(&trill_home, file_path).await;
    ...
    async fn loads_skills_from_system_cache_when_present() {
        ...
        let skill_path = write_system_skill(&trill_home, "system", "system-skill", "from system");
