...
impl SkillsManager {
    pub fn new(trill_home: PathBuf) -> Self {
        if let Err(err) = install_system_skills(&trill_home) {
...
mod tests {
    ...
    fn write_user_skill(trill_home: &TempDir, dir: &str, name: &str, description: &str) {
        let skill_dir = trill_home.path().join("skills").join(dir);
    ...
    async fn skills_for_config_seeds_cache_by_cwd() {
        ...
        let skills_manager = SkillsManager::new(trill_home.path().to_path_buf());

        write_user_skill(&trill_home, "a", "skill-a", "from a");
        ...
        write_user_skill(&trill_home, "b", "skill-b", "from b");
