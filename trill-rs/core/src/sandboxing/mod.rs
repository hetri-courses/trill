...
pub use trill_protocol::models::SandboxPermissions;
...
pub(crate) enum SandboxTransformError {
    #[error("missing trill-linux-sandbox executable path")]
    ...
    pub(crate) fn transform(
        ...
        let (command, sandbox_env, arg0_override) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let exe = trill_linux_sandbox_exe
                ...
                (
                    full_command,
                    HashMap::new(),
                    Some("trill-linux-sandbox".to_string()),
                )
            }
            // On Windows, the restricted token sandbox executes in-process via the
            // trill-windows-sandbox crate. We leave the command unchanged here and
