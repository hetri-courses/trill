...
use trill_app_server_protocol::AuthMode as ApiAuthMode;
...
use trill_client::CodexHttpClient;
use trill_protocol::account::PlanType as AccountPlanType;
...
impl CodexAuth {
    fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(trill_home.to_path_buf(), storage_mode);
    ...
    pub fn from_auth_storage(
        ...
        load_auth(trill_home, false, auth_credentials_store_mode)
    }
...
pub fn read_trill_api_key_from_env() -> Option<String> {
...
/// Delete the auth.json file inside `trill_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    trill_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(trill_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(trill_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(trill_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(trill_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(trill_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(trill_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

fn load_auth(
    trill_home: &Path,
    enable_trill_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<CodexAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        let client = crate::default_client::create_client();
        CodexAuth::from_auth_dot_json(trill_home, auth_dot_json, storage_mode, client)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_trill_api_key_env && let Some(api_key) = read_trill_api_key_from_env() {
        let client = crate::default_client::create_client();
...
impl AuthManager {
    ...
    pub fn from_auth_for_testing(auth: CodexAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            trill_home: PathBuf::from("non-existent"),
            inner: RwLock::new(cached),
            enable_trill_api_key_env: false,
    ...
    pub fn from_auth_for_testing_with_home(auth: CodexAuth, trill_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            external_refresher: None,
        };
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.trill_home, self.auth_credentials_store_mode)?;
...
mod tests {
    ...
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let auth = super::load_auth(trill_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, trill_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(trill_home);
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        let trill_home = tempdir().unwrap();
        login_with_api_key(trill_home.path(), "sk-test", AuthCredentialsStoreMode::File)
            .expect("seed api key");

        let config = build_config(trill_home.path(), Some(ForcedLoginMethod::Chatgpt), None).await;

        let err = super::enforce_login_restrictions(&config)
            .expect_err("expected method mismatch to error");
        assert!(err.to_string().contains("ChatGPT login is required"));
        assert!(
            !trill_home.path().join("auth.json").exists(),
    ...
    async fn enforce_login_restrictions_logs_out_for_workspace_mismatch() {
        ...
        let config = build_config(trill_home.path(), None, Some("org_mine".to_string())).await;
