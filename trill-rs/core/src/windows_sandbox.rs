...
pub fn sandbox_setup_is_complete(trill_home: &Path) -> bool {
    trill_windows_sandbox::sandbox_setup_is_complete(trill_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_trill_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = trill_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = trill_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn run_elevated_setup(
    ...
    trill_windows_sandbox::run_elevated_setup(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
