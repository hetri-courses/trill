...
async fn make_config_for_test(
    ...
    tokio::fs::write(
        trill_home.join(CONFIG_TOML_FILE),
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .trill_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = trill_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let trill_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&trill_home).await?;
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".trill")).await?;
    tokio::fs::create_dir_all(project_root.join(".trill")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".trill").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".trill").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let trill_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&trill_home).await?;
    make_config_for_test(&trill_home, &project_root, TrustLevel::Trusted, None).await?;
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(".trill").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".trill").as_path()
    );
...
"#;
    tokio::fs::write(project_root.join(".trill").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".trill").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".trill").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".trill").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let trill_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&trill_home).await?;
    tokio::fs::write(trill_home.join(CONFIG_TOML_FILE), "").await?;
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".trill"))?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
            disabled_reason: None,
...
#[tokio::test]
async fn trill_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let trill_home = home_dir.join(".trill");
    tokio::fs::create_dir_all(&trill_home).await?;
    tokio::fs::write(trill_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn trill_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".trill");
    let nested_dot_codex = nested.join(".trill");
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let trill_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&trill_home_untrusted).await?;
    make_config_for_test(
        &trill_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        None,
    )
    .await?;
    let untrusted_config_path = trill_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let trill_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&trill_home_unknown).await?;
    tokio::fs::write(
        trill_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".trill")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".trill").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let trill_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&trill_home).await?;
        let config_path = trill_home.join(CONFIG_TOML_FILE);

        if let Some(trust_level) = trust_level {
            make_config_for_test(&trill_home, &project_root, trust_level, None).await?;
...
mod requirements_exec_policy_tests {
    ...
    use trill_execpolicy::Decision;
    use trill_execpolicy::Evaluation;
    use trill_execpolicy::RuleMatch;
    use trill_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
