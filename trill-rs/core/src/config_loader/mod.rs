...
/// - tree      parent directories up to root looking for `./.trill/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.trill/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers = layer_io::load_config_layers_internal(trill_home, overrides).await?;
    ...
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, trill_home)?;
...
async fn load_project_layers(
    ...
    let trill_home_abs = AbsolutePathBuf::from_absolute_path(trill_home)?;
    let trill_home_normalized =
        normalize_path(trill_home_abs.as_path()).unwrap_or_else(|_| trill_home_abs.to_path_buf());
    let mut dirs = cwd
        .as_path()
        .ancestors()
        .scan(false, |done, a| {
    ...
    for dir in dirs {
        ...
        if dot_codex_abs == trill_home_abs || dot_codex_normalized == trill_home_normalized {
            continue;
        }
