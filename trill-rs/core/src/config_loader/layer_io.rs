...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(trill_home)),
    )?;
...
pub(super) fn managed_config_default_path(trill_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = trill_home;
    ...
    #[cfg(not(unix))]
    {
        trill_home.join("managed_config.toml")
    }
}
