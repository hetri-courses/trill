...
This module is the canonical place to **load and describe Trill configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `trill_core::config_loader`:
...
```rust
use trill_core::config_loader::{load_config_layers_state, LoaderOverrides};
