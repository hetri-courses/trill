...
mod tests {
    ...
    fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
        ...
        // top-level .git and .trill directories and one without them.
        ...
        // write to .trill/config.toml in the vulnerable root.
        ...
        // - write access to WRITABLE_ROOT_0 (but not its .git or .trill), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
"#,
        ...
        // Verify that .trill/config.toml cannot be modified under the generated
        // Seatbelt policy.
        ...
        // Verify that writing a file to the folder containing .git and .trill is allowed.
    ...
    fn create_seatbelt_args_for_cwd_as_git_repo() {
        ...
        // `.trill` checks are done properly for cwd.
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: false,
...
"#,
    ...
    struct PopulatedTmp {
        /// Path containing a .git and .trill subfolder.
        ...
        /// ran `git commit` themselves, or modified .trill/config.toml to
        ...
        /// Path without .git or .trill subfolders.
        empty_root: PathBuf,
    ...
    fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
        ...
        fs::create_dir_all(vulnerable_root.join(".trill")).expect("create .trill");
        fs::write(
            vulnerable_root.join(".trill").join("config.toml"),
            "sandbox_mode = \"read-only\"\n",
        )
        .expect("write .trill/config.toml");
        ...
        let dot_codex_canonical = vulnerable_root_canonical.join(".trill");
