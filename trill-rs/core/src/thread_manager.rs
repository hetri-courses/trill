...
pub struct ThreadManager {
    ...
    #[cfg(any(test, feature = "test-support"))]
    _test_trill_home_guard: Option<TempDir>,
}
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<TrillThread>>>>,
...
impl ThreadManager {
    ...
    pub fn with_models_provider(auth: CodexAuth, provider: ModelProviderInfo) -> Self {
        ...
        let mut manager = Self::with_models_provider_and_home(auth, provider, trill_home);
        manager._test_trill_home_guard = Some(temp_dir);
        manager
    }
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> CodexResult<Arc<TrillThread>> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<trill_protocol::dynamic_tools::DynamicToolSpec>,
    ...
    /// as `Arc<TrillThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<TrillThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> CodexResult<Arc<TrillThread>> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<TrillThread>> {
    ...
    async fn finalize_thread_spawn(
        ...
        let thread = Arc::new(TrillThread::new(
            codex,
