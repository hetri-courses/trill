...
use trill_state::DB_METRIC_COMPARE_ERROR;
pub use trill_state::LogEntry;
use trill_state::STATE_DB_FILENAME;
use trill_state::ThreadMetadataBuilder;
...
pub type StateDbHandle = Arc<trill_state::StateRuntime>;
...
pub(crate) async fn init_if_enabled(
    ...
    let state_path = config.trill_home.join(STATE_DB_FILENAME);
    if !config.features.enabled(Feature::Sqlite) {
        return None;
    }
    let existed = tokio::fs::try_exists(&state_path).await.unwrap_or(false);
    let runtime = match trill_state::StateRuntime::init(
...
pub async fn open_if_present(trill_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = trill_home.join(STATE_DB_FILENAME);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime = trill_state::StateRuntime::init(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<trill_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    ...
    let ctx = context?;
    if ctx.trill_home() != trill_home {
        warn!(
            "state db trill_home mismatch: expected {}, got {}",
            ctx.trill_home().display(),
            trill_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => trill_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => trill_state::SortKey::UpdatedAt,
            },
...
pub fn record_discrepancy(stage: &str, reason: &str) {
    ...
    if let Some(metric) = trill_otel::metrics::global() {
