...
use trill_app_server_protocol::Tools;
use trill_app_server_protocol::UserSavedConfig;
use trill_protocol::config_types::AltScreenMode;
...
use trill_protocol::config_types::SandboxMode;
use trill_protocol::config_types::TrustLevel;
...
use trill_utils_absolute_path::AbsolutePathBufGuard;
...
pub use trill_git::GhostSnapshotConfig;
...
pub(crate) fn test_config() -> Config {
    let trill_home = tempdir().expect("create temp dir");
...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.trill/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Trill"]
    /// ```
    ///
    ...
    ///          https://github.com/hetri-courses/trill/blob/main/trill-rs/rmcp-client/src/oauth.rs#L2
    ...
    /// Directory containing all Codex state (defaults to `~/.trill` but can be
    /// overridden by the `CODEX_HOME` environment variable).
    pub trill_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.trill/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `trill-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `trill-linux-sandbox`.
    pub trill_linux_sandbox_exe: Option<PathBuf>,
...
impl ConfigBuilder {
    pub fn trill_home(mut self, trill_home: PathBuf) -> Self {
        self.trill_home = Some(trill_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let trill_home = trill_home.map_or_else(find_trill_home, std::io::Result::Ok)?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let trill_home = find_trill_home()?;
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &trill_home)?;
    ...
    /// in [ConfigToml], such as `cwd` and `trill_linux_sandbox_exe`.
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, trill_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .trill/ folders.
...
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder::new(trill_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(trill_home: &Path, provider: &str) -> std::io::Result<()> {
...
/// Base config deserialized from ~/.trill/config.toml.
...
pub struct ConfigToml {
    ...
    /// directories for `.trill` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
impl ConfigToml {
    ...
    fn derive_sandbox_policy(
        ...
        if cfg!(target_os = "windows")
            ...
            && windows_sandbox_level == trill_protocol::config_types::WindowsSandboxLevel::Disabled
        {
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, trill_home, config_layer_stack)
    }
    ...
    fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&trill_home));
...
///   directory exists.
pub fn find_trill_home() -> std::io::Result<PathBuf> {
    trill_utils_home_dir::find_trill_home()
}

/// Returns the path to the folder where Codex logs are stored. Does not verify
/// that the directory exists.
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.trill_home.clone();
    p.push("log");
    Ok(p)
}
...
"#,
    ...
    async fn project_profile_overrides_user_profile() -> std::io::Result<()> {
        ...
        std::fs::write(
            trill_home.path().join(CONFIG_TOML_FILE),
            format!(
                r#"
profile = "global"
...
"#,
        ...
        let project_config_dir = workspace.path().join(".trill");
...
"#,
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let trill_home = TempDir::new()?;
        let managed_path = trill_home.path().join("managed_config.toml");
        let config_path = trill_home.path().join(CONFIG_TOML_FILE);
        ...
        let cwd = AbsolutePathBuf::try_from(trill_home.path())?;
        let config_layer_stack = load_config_layers_state(
            trill_home.path(),
            Some(cwd),
            &Vec::new(),
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(trill_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(trill_home.path()).await?;
...
"#,
        ...
        let err = load_global_mcp_servers(trill_home.path())
            .await
...
"#
    ...
    async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
        ...
        let cwd_path = PathBuf::from("/tmp/trill-mcp");
        let servers = BTreeMap::from([(
            "docs".to_string(),
            McpServerConfig {
                transport: McpServerTransportConfig::Stdio {
        ...
        let serialized = std::fs::read_to_string(&config_path)?;
        assert!(
            serialized.contains(r#"cwd = "/tmp/trill-mcp""#),
        ...
        match &docs.transport {
            McpServerTransportConfig::Stdio { cwd, .. } => {
                assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/trill-mcp")));
            }
            other => panic!("unexpected transport {other:?}"),
        }
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(trill_home.path())
            .set_model(Some("gpt-5.1-codex"), Some(ReasoningEffort::High))
            .apply()
            .await?;

        let serialized =
            tokio::fs::read_to_string(trill_home.path().join(CONFIG_TOML_FILE)).await?;
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn trill_home(&self) -> PathBuf {
            self.trill_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let trill_home = TempDir::new()?;
        let workspace = trill_home.path().join("workspace");
...
"#;
        ...
        let trill_home_temp_dir = TempDir::new().unwrap();
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            trill_home: trill_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                trill_home: fixture.trill_home(),
                config_layer_stack: Default::default(),
                history: History::default(),
                ephemeral: false,
                file_opener: UriBasedFileOpener::VsCode,
                trill_linux_sandbox_exe: None,
...
"#;
    ...
    fn test_set_default_oss_provider() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let trill_home = temp_dir.path();
        let config_path = trill_home.join(CONFIG_TOML_FILE);

        // Test setting valid provider on empty config
        set_default_oss_provider(trill_home, OLLAMA_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        assert!(content.contains("oss_provider = \"ollama\""));

        // Test updating existing config
        std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
        set_default_oss_provider(trill_home, LMSTUDIO_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        ...
        let result = set_default_oss_provider(trill_home, "invalid_provider");
