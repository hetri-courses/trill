...
use trill_app_server_protocol::Config as ApiConfig;
use trill_app_server_protocol::ConfigBatchWriteParams;
use trill_app_server_protocol::ConfigLayerMetadata;
...
use trill_app_server_protocol::OverriddenMetadata;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(trill_home: PathBuf) -> Self {
        Self {
            trill_home,
        ...
        let layers = match params.cwd.as_deref() {
            Some(cwd) => {
                ...
                crate::config::ConfigBuilder::default()
                    .trill_home(self.trill_home.clone())
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<trill_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.trill_home)
        ...
        if !config_edits.is_empty() {
            ConfigEditsBuilder::new(&self.trill_home)
    ...
    /// include any in-repo .trill/ folders because there is no cwd/project root
