...
pub async fn apply(
    ...
    let trill_home = trill_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&trill_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(trill_home: &Path) -> Self {
        Self {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.trill_home, self.profile.as_deref(), &self.edits)
    }
...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        let contents =
            std::fs::read_to_string(trill_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#,
        ...
        let raw = std::fs::read_to_string(trill_home.join(CONFIG_TOML_FILE)).expect("read config");
    ...
    fn blocking_set_model_writes_through_symlink_chain() {
        ...
        let link_path = trill_home.join("config-link.toml");
        let config_path = trill_home.join(CONFIG_TOML_FILE);
...
"#;
    ...
    fn blocking_set_model_replaces_symlink_on_cycle() {
        let tmp = tempdir().expect("tmpdir");
        let trill_home = tmp.path();
        let link_a = trill_home.join("a.toml");
        let link_b = trill_home.join("b.toml");
        let config_path = trill_home.join(CONFIG_TOML_FILE);
...
"#;
        std::fs::write(trill_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
"#,
        ...
        apply_blocking(
            ...
            None,
            &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
                "hide_gpt-5.1-trill-max_migration_prompt".to_string(),
                true,
            )],
        )
        .expect("persist");
        ...
        let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-trill-max_migration_prompt" = true
"#;
        assert_eq!(contents, expected);
    }
...
"#,
        ...
        apply_blocking(trill_home, None, &[ConfigEdit::ReplaceMcpServers(servers)])
            .expect("persist");
...
"#;
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !trill_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
        ...
        contents = std::fs::read_to_string(trill_home.join(CONFIG_TOML_FILE)).expect("read config");
