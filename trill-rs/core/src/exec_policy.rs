...
use trill_execpolicy::AmendError;
use trill_execpolicy::Decision;
use trill_execpolicy::Error as ExecPolicyRuleError;
use trill_execpolicy::Evaluation;
use trill_execpolicy::Policy;
use trill_execpolicy::PolicyParser;
use trill_execpolicy::RuleMatch;
use trill_execpolicy::blocking_append_allow_prefix_rule;
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(trill_home);
...
pub async fn load_exec_policy(config_stack: &ConfigLayerStack) -> Result<Policy, ExecPolicyError> {
    ...
    // Include disabled project layers so .trill/rules still applies when
...
fn default_policy_path(trill_home: &Path) -> PathBuf {
    trill_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
"#;
    ...
    async fn append_execpolicy_amendment_updates_policy_and_file() {
        ...
        manager
            .append_amendment_and_update(trill_home.path(), &ExecPolicyAmendment::from(prefix))
            .await
        ...
        let contents = fs::read_to_string(default_policy_path(trill_home.path()))
...
"#
    ...
    async fn append_execpolicy_amendment_rejects_empty_prefix() {
        ...
        let result = manager
            .append_amendment_and_update(trill_home.path(), &ExecPolicyAmendment::from(vec![]))
            .await;
