...
use trill_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: trill_protocol::config_types::WindowsSandboxLevel,
...
pub async fn process_exec_tool_call(
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            trill_linux_sandbox_exe.as_ref(),
            windows_sandbox_level,
        )
...
fn record_windows_sandbox_spawn_failure(
    ...
    windows_sandbox_level: trill_protocol::config_types::WindowsSandboxLevel,
    err: &str,
) {
    ...
    let level = if matches!(
        windows_sandbox_level,
        trill_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = trill_otel::metrics::global() {
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_trill_home;
    use trill_protocol::config_types::WindowsSandboxLevel;
    use trill_windows_sandbox::run_windows_sandbox_capture;
    use trill_windows_sandbox::run_windows_sandbox_capture_elevated;
    ...
    let trill_home = find_trill_home().map_err(|err| {
        CodexErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve trill_home: {err}"
        )))
    })?;
