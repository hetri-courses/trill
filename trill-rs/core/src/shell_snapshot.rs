...
impl ShellSnapshot {
    pub fn start_snapshotting(
        ...
        tokio::spawn(
            async move {
                ...
                let snapshot =
                    ShellSnapshot::try_new(&trill_home, snapshot_session_id, &snapshot_shell)
                        .await
                        .map(Arc::new);
    ...
    async fn try_new(trill_home: &Path, session_id: ThreadId, shell: &Shell) -> Option<Self> {
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&trill_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            trill_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(trill_home: &Path, active_session_id: ThreadId) -> Result<()> {
    let snapshot_dir = trill_home.join(SNAPSHOT_DIR);
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(trill_home, session_id).await?;
...
mod tests {
    ...
    fn bash_snapshot_filters_invalid_exports() -> Result<()> {
        let output = Command::new("/bin/bash")
            ...
            .env("VALID_NAME", "ok")
            .env("PWD", "/tmp/stale")
            .env("NEXTEST_BIN_EXE_trill-write-config-schema", "/path/to/bin")
            .env("BAD-NAME", "broken")
            .output()?;
        ...
        assert!(!stdout.contains("NEXTEST_BIN_EXE_trill-write-config-schema"));
    ...
    async fn write_rollout_stub(trill_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
        let dir = trill_home
            .join("sessions")
            .join("2025")
            .join("01")
    ...
    async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(trill_home, stale_session).await?;
    ...
    async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(trill_home, active_session).await?;
        fs::write(&active_snapshot, "active").await?;

        set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

        cleanup_stale_snapshots(trill_home, active_session).await?;
