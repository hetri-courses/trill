...
use trill_keyring_store::DefaultKeyringStore;
use trill_keyring_store::KeyringStore;
...
pub(super) fn get_auth_file(trill_home: &Path) -> PathBuf {
    trill_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(trill_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(trill_home: PathBuf) -> Self {
        Self { trill_home }
    }
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.trill_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.trill_home)
    }
}

const KEYRING_SERVICE: &str = "Codex Auth";

// turns trill_home path into a stable, short key string
fn compute_store_key(trill_home: &Path) -> std::io::Result<String> {
    let canonical = trill_home
        .canonicalize()
        .unwrap_or_else(|_| trill_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(trill_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            trill_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.trill_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.trill_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.trill_home)?;
...
impl AutoAuthStorage {
    fn new(trill_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(trill_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(trill_home)),
        }
    }
}
...
// A global in-memory store for mapping trill_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(trill_home: PathBuf) -> Self {
        Self { trill_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(trill_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(trill_home)),
        AuthCredentialsStoreMode::Keyring => {
            Arc::new(KeyringAuthStorage::new(trill_home, keyring_store))
        }
        AuthCredentialsStoreMode::Auto => Arc::new(AutoAuthStorage::new(trill_home, keyring_store)),
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(trill_home)),
    }
}
...
mod tests {
    ...
    use trill_keyring_store::tests::MockKeyringStore;
    use keyring::Error as KeyringError;

    #[tokio::test]
    async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
        let trill_home = tempdir()?;
        let storage = FileAuthStorage::new(trill_home.path().to_path_buf());
    ...
    async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
        ...
        let file = get_auth_file(trill_home.path());
        storage
            .save(&auth_dot_json)
            .context("failed to save auth file")?;
    ...
    fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
        ...
        seed_keyring_with_auth(
            &mock_keyring,
            || compute_store_key(trill_home.path()),
            &expected,
        )?;
    ...
    fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
        let trill_home = PathBuf::from("~/.trill");

        let key = compute_store_key(trill_home.as_path())?;
    ...
    fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
        ...
        let auth_file = get_auth_file(trill_home.path());
        std::fs::write(&auth_file, "stale")?;
        ...
        let key = compute_store_key(trill_home.path())?;
    ...
    fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
        let trill_home = tempdir()?;
        let mock_keyring = MockKeyringStore::default();
        let storage = AutoAuthStorage::new(trill_home.path().to_path_buf(), Arc::new(mock_keyring));
