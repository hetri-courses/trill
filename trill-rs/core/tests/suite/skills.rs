...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let skill_path = test.trill_home_path().join("skills/demo/SKILL.md");
    ...
    core_test_support::wait_for_event(test.trill.as_ref(), |event| {
        matches!(event, trill_core::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn skill_load_errors_surface_in_session_configured() -> Result<()> {
    ...
    test.trill
        ...
        .await?;
    let response =
        core_test_support::wait_for_event_match(test.trill.as_ref(), |event| match event {
            trill_core::protocol::EventMsg::ListSkillsResponse(response) => Some(response.clone()),
            _ => None,
        })
        .await;
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.trill_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, trill_protocol::protocol::SkillScope::System);
