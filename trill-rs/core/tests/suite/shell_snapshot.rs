...
async fn wait_for_snapshot(trill_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = trill_home.join("shell_snapshots");
...
async fn run_snapshot_command(command: &str) -> Result<SnapshotRun> {
    ...
    let trill_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&trill_home).await?;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.trill_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let trill_home = home.path().to_path_buf();
    let codex = harness.test().trill.clone();
