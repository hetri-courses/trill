...
use trill_core::protocol::COLLABORATION_MODE_CLOSE_TAG;
use trill_core::protocol::COLLABORATION_MODE_OPEN_TAG;
...
async fn override_turn_context_records_permissions_update() -> Result<()> {
    ...
    test.trill.submit(Op::Shutdown).await?;
    wait_for_event(&test.trill, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
