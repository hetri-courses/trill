...
use trill_core::REVIEW_PROMPT;
use trill_core::ResponseItem;
use trill_core::config::Config;
use trill_core::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use trill_core::protocol::EventMsg;
use trill_core::protocol::ExitedReviewModeEvent;
use trill_core::protocol::Op;
use trill_core::protocol::ReviewCodeLocation;
use trill_core::protocol::ReviewFinding;
use trill_core::protocol::ReviewLineRange;
use trill_core::protocol::ReviewOutputEvent;
use trill_core::protocol::ReviewRequest;
use trill_core::protocol::ReviewTarget;
use trill_core::protocol::RolloutItem;
use trill_core::protocol::RolloutLine;
use trill_core::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    ...
    let codex = new_conversation_for_server(&server, trill_home.clone(), |_| {}).await;
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let codex = new_conversation_for_server(&server, trill_home.clone(), |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = trill_home.path().join("resume.jsonl");
    {
        ...
        let user = trill_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = trill_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let codex =
        resume_conversation_for_server(&server, trill_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let codex = new_conversation_for_server(&server, trill_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, trill_home, resume_path)
        .await
