...
const REMOTE_MODEL_SLUG: &str = "trill-test";
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "trill-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "trill-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        1,
...
async fn remote_models_preserve_builtin_presets() -> Result<()> {
    ...
    let manager = ModelsManager::with_provider(
        ...
        trill_core::auth::AuthManager::from_auth_for_testing(auth),
        provider,
    );
    ...
    assert!(
        available
            .iter()
            .any(|model| model.model == "gpt-5.1-trill-max"),
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let remote_model = test_remote_model("trill-auto-balanced", ModelVisibility::Hide, 0);
    mount_models_once(
        &server,
        ModelsResponse {
            models: vec![remote_model],
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "trill-auto-balanced")
