...
use trill_protocol::protocol::UserMessageEvent;
...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let rollout_path = test.trill.rollout_path().expect("rollout path");
    let db_path = test.config.trill_home.join(STATE_DB_FILENAME);
    ...
    let db = test.trill.state_db().expect("state db enabled");
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_pre_build_hook(move |trill_home| {
            let rollout_path = trill_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.trill_home.join(STATE_DB_FILENAME);
    let rollout_path = test.config.trill_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let subscriber = tracing_subscriber::registry().with(trill_state::log_db::start(db.clone()));
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = trill_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
