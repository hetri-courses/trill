...
use trill_core::protocol::ExecOutputStream;
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.trill, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.trill, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.trill, |ev| match ev {
    ...
    let _ = wait_for_event(&test.trill, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
