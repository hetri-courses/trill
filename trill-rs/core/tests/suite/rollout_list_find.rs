...
use trill_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(trill_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = trill_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(trill_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(trill_home, "sessions", id)
}
...
#[tokio::test]
async fn find_handles_gitignore_covering_trill_home_directory() {
    let repo = TempDir::new().unwrap();
    let trill_home = repo.path().join(".trill");
    std::fs::create_dir_all(&trill_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".trill/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&trill_home, id);

    let found = find_thread_path_by_id_str(&trill_home, &id.to_string())
        .await
        .unwrap();
...
async fn find_locates_rollout_file_written_by_recorder() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::default()
        .trill_home(home.path().to_path_buf())
        .build()
        .await?;
