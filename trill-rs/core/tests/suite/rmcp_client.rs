...
use trill_utils_cargo_bin::cargo_bin;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.trill, |ev| {
    ...
    let end_event = wait_for_event(&fixture.trill, |ev| {
    ...
    wait_for_event(&fixture.trill, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn stdio_image_completions_round_trip() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        .with_config(move |config| {
            config.model_provider.wire_api = trill_core::WireApi::Chat;
            let mut servers = config.mcp_servers.get().clone();
            servers.insert(
                server_name.to_string(),
                McpServerConfig {
