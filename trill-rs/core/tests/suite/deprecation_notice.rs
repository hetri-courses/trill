...
use trill_core::config_loader::ConfigLayerEntry;
use trill_core::config_loader::ConfigLayerStack;
use trill_core::config_loader::ConfigRequirements;
...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://github.com/hetri-courses/trill/blob/main/docs/config.md#feature-flags for details."
        ),
    );
