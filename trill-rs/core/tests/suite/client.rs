...
use trill_core::ContentItem;
use trill_core::LocalShellAction;
use trill_core::LocalShellExecAction;
use trill_core::LocalShellStatus;
use trill_core::ModelClient;
use trill_core::ModelProviderInfo;
use trill_core::NewThread;
use trill_core::Prompt;
use trill_core::ResponseEvent;
use trill_core::ResponseItem;
use trill_core::ThreadManager;
use trill_core::TransportManager;
use trill_core::WireApi;
use trill_core::auth::AuthCredentialsStoreMode;
use trill_core::built_in_model_providers;
use trill_core::default_client::originator;
...
use trill_protocol::models::ReasoningItemContent;
...
/// Writes an `auth.json` into the provided `trill_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        trill_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = trill_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![trill_protocol::models::ContentItem::InputText {
    ...
    let prior_system = trill_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![trill_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = trill_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let trill_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codex()
        .with_home(trill_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, trill_home, session_path.clone())
        .await
        .expect("resume conversation");
    let codex = test.trill.clone();
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    // Init session
    let trill_home = TempDir::new().unwrap();
    ...
    let mut config = load_default_config_for_test(&trill_home).await;
    config.model_provider = model_provider;

    let auth_manager =
        match CodexAuth::from_auth_storage(trill_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => trill_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No CodexAuth found in trill_home"),
...
async fn skills_append_to_instructions() {
    ...
    let trill_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = trill_home.path().join("skills/demo");
    ...
    let trill_home_path = trill_home.path().to_path_buf();
    let mut builder = test_codex()
        .with_home(trill_home.clone())
        .with_auth(CodexAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = trill_home_path;
        });
    let codex = builder
        .build(&server)
        .await
    ...
    let _trill_home_guard = trill_home;
}
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-trill-primary-used-percent", "12.5")
        .insert_header("x-trill-secondary-used-percent", "40.0")
        .insert_header("x-trill-primary-window-minutes", "10")
        .insert_header("x-trill-secondary-window-minutes", "60")
        .insert_header("x-trill-primary-reset-at", "1704069000")
        .insert_header("x-trill-secondary-reset-at", "1704074400")
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5.1-trill-max in tests → 95% usable context window
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-trill-primary-used-percent", "100.0")
        .insert_header("x-trill-secondary-used-percent", "87.5")
        .insert_header("x-trill-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-trill-primary-window-minutes", "15")
        .insert_header("x-trill-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let codex = codex_fixture.trill.clone();
