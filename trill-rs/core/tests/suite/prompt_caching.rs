...
use trill_apply_patch::APPLY_PATCH_TOOL_INSTRUCTIONS;
use trill_core::features::Feature;
use trill_core::models_manager::model_info::BASE_INSTRUCTIONS;
...
use trill_core::shell::Shell;
use trill_core::shell::default_user_shell;
...
async fn prompt_tools_are_consistent_across_requests() -> anyhow::Result<()> {
    ...
    let TestCodex {
        ...
        .with_config(|config| {
            ...
            config.model = Some("gpt-5.1-trill-max".to_string());
...
async fn codex_mini_latest_tools() -> anyhow::Result<()> {
    ...
    let TestCodex { codex, .. } = test_codex()
        .with_config(|config| {
            ...
            config.features.enable(Feature::CollaborationModes);
            config.model = Some("trill-mini-latest".to_string());
        })
        .build(&server)
        .await?;
