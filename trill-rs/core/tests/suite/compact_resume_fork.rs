...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<TrillThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.trill)
}

async fn user_turn(conversation: &Arc<TrillThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<TrillThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<TrillThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = trill_core::AuthManager::from_auth_for_testing(
        trill_core::CodexAuth::from_api_key("dummy"),
    );
    manager
