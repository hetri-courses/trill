...
use trill_core::auth::CODEX_API_KEY_ENV_VAR;
use trill_core::protocol::GitInfo;
...
fn repo_root() -> std::path::PathBuf {
    ...
    trill_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn chat_mode_stream_cli() {
    ...
    let bin = trill_utils_cargo_bin::cargo_bin("codex").unwrap();
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the trill CLI and invoke `exec`, which is what records a session.
    let bin = trill_utils_cargo_bin::cargo_bin("codex").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = trill_utils_cargo_bin::cargo_bin("codex").unwrap();
    ...
    assert!(output2.status.success(), "resume trill-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = trill_core::git_info::collect_git_info(&git_repo).await;
