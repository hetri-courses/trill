use trill_core::config::types::Personality;
...
use trill_protocol::openai_models::ModelInstructionsVariables;
use trill_protocol::openai_models::ModelMessages;
...
async fn model_personality_does_not_mutate_base_instructions_without_template() {
    let trill_home = TempDir::new().expect("create temp dir");
...
async fn user_turn_personality_none_does_not_add_update_message() -> anyhow::Result<()> {
    ...
    wait_for_event(&test.trill, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_codex()
        .with_model("exp-trill-personality")
        .with_config(|config| {
            config.features.disable(Feature::RemoteModels);
            config.features.enable(Feature::Personality);
        });
...
async fn ignores_remote_model_personality_if_remote_models_disabled() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex()
        .with_auth(trill_core::CodexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config.features.disable(Feature::RemoteModels);
            config.features.enable(Feature::Personality);
...
async fn remote_model_default_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "trill-remote-default-personality";
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "trill-remote-personality";
    let remote_personality_message = "Friendly from remote template";
    let remote_model = ModelInfo {
        slug: remote_slug.to_string(),
        display_name: "Remote personality test".to_string(),
...
async fn wait_for_model_available(
    ...
    slug: &str,
    config: &trill_core::config::Config,
) {
