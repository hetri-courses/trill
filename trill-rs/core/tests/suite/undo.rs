...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.email", "trill-tests@example.com"])?;
...
async fn invoke_undo(codex: &Arc<TrillThread>) -> Result<UndoCompletedEvent> {
...
async fn expect_successful_undo(codex: &Arc<TrillThread>) -> Result<UndoCompletedEvent> {
...
async fn expect_failed_undo(codex: &Arc<TrillThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(codex).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let codex = Arc::clone(&harness.test().trill);
