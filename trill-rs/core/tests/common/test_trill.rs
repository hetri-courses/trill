...
impl TestCodexBuilder {
    ...
    async fn build_from_config(
        ...
        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = trill_core::AuthManager::from_auth_for_testing(auth);
                thread_manager
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = trill_utils_cargo_bin::cargo_bin("codex") {
            config.trill_linux_sandbox_exe = Some(path);
        }
...
impl TestCodex {
    ...
    pub fn trill_home_path(&self) -> &Path {
