...
use trill_utils_cargo_bin::CargoBinError;
use trill_utils_cargo_bin::find_resource;
...
/// avoids clobbering a developer’s real `~/.trill`.
pub async fn load_default_config_for_test(trill_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        trill_linux_sandbox_exe: Some(
            trill_utils_cargo_bin::cargo_bin("trill-linux-sandbox")
                .expect("should find binary for trill-linux-sandbox"),
        ),
...
pub async fn wait_for_event<F>(codex: &TrillThread, predicate: F) -> trill_core::protocol::EventMsg
where
    F: FnMut(&trill_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(codex, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(codex: &TrillThread, matcher: F) -> T
where
    F: Fn(&trill_core::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    trill_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    trill_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    trill_core::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    trill_core::shell::default_user_shell().derive_exec_args(command, false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    trill_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
