...
use trill_core::is_dangerous_command::command_might_be_dangerous;
...
const CODEX_EXECVE_WRAPPER_EXE_NAME: &str = "trill-execve-wrapper";
...
async fn load_exec_policy() -> anyhow::Result<Policy> {
    let trill_home = find_trill_home().context("failed to resolve trill_home for execpolicy")?;

    // TODO(mbolin): At a minimum, `cwd` should be configurable via
    // `codex/sandbox-state/update` or some other custom MCP call.
    let cwd = None;
    let cli_overrides = Vec::new();
    let overrides = trill_core::config_loader::LoaderOverrides::default();
    let config_layer_stack = trill_core::config_loader::load_config_layers_state(
        &trill_home,
        cwd,
        &cli_overrides,
        overrides,
        trill_core::config_loader::CloudRequirementsLoader::default(),
    )
    .await?;

    trill_core::load_exec_policy(&config_layer_stack)
        .await
