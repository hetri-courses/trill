use trill_core::MCP_SANDBOX_STATE_METHOD;
use trill_core::SandboxState;
...
where
    P: AsRef<Path>,
{
    let mcp_executable = trill_utils_cargo_bin::cargo_bin("trill-exec-mcp-server")?;
    let execve_wrapper = trill_utils_cargo_bin::cargo_bin("trill-execve-wrapper")?;
    ...
    let transport = TokioChildProcess::new(Command::new(&mcp_executable).configure(|cmd| {
        ...
        cmd.env("CODEX_HOME", trill_home.as_ref());
...
pub async fn write_default_execpolicy<P>(policy: &str, trill_home: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    let policy_dir = trill_home.as_ref().join("rules");
