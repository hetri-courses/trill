...
use anyhow::Result;
use anyhow::ensure;
use trill_exec_server::ExecResult;
...
"#,
    ...
    let transport = create_transport(trill_home.as_ref(), dotslash_cache).await?;
    ...
    let trill_linux_sandbox_exe = if cfg!(target_os = "linux") {
        let trill_linux_sandbox_exe = linux_sandbox_exe_folder.path().join("trill-linux-sandbox");
        let trill_cli = ensure_trill_cli()?;
        symlink(&trill_cli, &trill_linux_sandbox_exe)?;
        Some(trill_linux_sandbox_exe)
    } else {
        None
    };
    let response =
        notify_readable_sandbox(&project_root_path, trill_linux_sandbox_exe, &service).await?;
...
fn ensure_trill_cli() -> Result<PathBuf> {
    let trill_cli = trill_utils_cargo_bin::cargo_bin("codex")?;

    let metadata = trill_cli.metadata().with_context(|| {
        format!(
        ...
        metadata.is_file(),
        "expected codex binary at {} to be a file; run `cargo build -p trill-cli --bin codex` before this test",
        trill_cli.display()
    );
    ...
    let mode = metadata.permissions().mode();
        ...
        mode & 0o111 != 0,
        "codex binary at {} is not executable (mode {mode:o}); run `cargo build -p trill-cli --bin codex` before this test",
        trill_cli.display()
    );
