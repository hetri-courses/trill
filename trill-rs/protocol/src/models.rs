...
use trill_utils_image::load_and_resize_to_fit;
...
use trill_utils_image::error::ImageProcessingError;
...
// (Moved event mapping logic into trill-core to avoid coupling protocol to UI-facing events.)
    ...
    fn includes_request_rule_instructions_when_enabled() {
        ...
        exec_policy
            .add_prefix_rule(
                ...
                trill_execpolicy::Decision::Allow,
            )
            .expect("add rule");
        let instructions = DeveloperInstructions::from_permissions_with_network(
            SandboxMode::WorkspaceWrite,
