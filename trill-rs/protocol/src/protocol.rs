...
pub enum Op {
    ...
    /// for a turn of a [`crate::trill_thread::TrillThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by trill-core; it does not
    /// involve the model.
...
/// privileges of the agent (e.g. `.trill`, `.git`, notably `.git/hooks`) under
                ...
                roots
                    ...
                    .map(|writable_root| {
                        ...
                        let top_level_codex = writable_root
                            .join(".trill")
                            .expect(".trill is a valid relative path");
