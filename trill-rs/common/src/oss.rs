...
pub fn get_default_model_for_oss_provider(provider_id: &str) -> Option<&'static str> {
    match provider_id {
        LMSTUDIO_OSS_PROVIDER_ID => Some(trill_lmstudio::DEFAULT_OSS_MODEL),
        OLLAMA_OSS_PROVIDER_ID | OLLAMA_CHAT_PROVIDER_ID => Some(trill_ollama::DEFAULT_OSS_MODEL),
        _ => None,
    }
}
...
pub async fn ollama_chat_deprecation_notice(
    ...
    if let Some(detection) = trill_ollama::detect_wire_api(&config.model_provider).await?
...
pub async fn ensure_oss_provider_ready(
    ...
    match provider_id {
        LMSTUDIO_OSS_PROVIDER_ID => {
            trill_lmstudio::ensure_oss_ready(config)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        OLLAMA_OSS_PROVIDER_ID | OLLAMA_CHAT_PROVIDER_ID => {
            trill_ollama::ensure_oss_ready(config)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        _ => {
