...
use chrono::DateTime;
use clap::Parser;
use trill_state::LogQuery;
use trill_state::LogRow;
use trill_state::STATE_DB_FILENAME;
use trill_state::StateRuntime;
use dirs::home_dir;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "trill-state-logs")]
#[command(about = "Tail Codex logs from state.sqlite with simple filters")]
struct Args {
    /// Path to CODEX_HOME. Defaults to $CODEX_HOME or ~/.trill.
    #[arg(long, env = "CODEX_HOME")]
    trill_home: Option<PathBuf>,

    /// Direct path to the SQLite database. Overrides --trill-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let trill_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(trill_home, "logs-client".to_string(), None).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let trill_home = args.trill_home.clone().unwrap_or_else(default_trill_home);
    Ok(trill_home.join(STATE_DB_FILENAME))
}

fn default_trill_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".trill");
    }
    PathBuf::from(".trill")
}
