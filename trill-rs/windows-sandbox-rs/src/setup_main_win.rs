...
use base64::Engine;
use trill_windows_sandbox::convert_string_sid_to_sid;
use trill_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use trill_windows_sandbox::ensure_allow_write_aces;
use trill_windows_sandbox::extract_setup_failure;
use trill_windows_sandbox::hide_newly_created_users;
use trill_windows_sandbox::load_or_create_cap_sids;
use trill_windows_sandbox::log_note;
use trill_windows_sandbox::path_mask_allows;
use trill_windows_sandbox::sandbox_dir;
use trill_windows_sandbox::sandbox_secrets_dir;
use trill_windows_sandbox::string_from_sid_bytes;
use trill_windows_sandbox::to_wide;
use trill_windows_sandbox::write_setup_error_report;
use trill_windows_sandbox::SetupErrorCode;
use trill_windows_sandbox::SetupErrorReport;
use trill_windows_sandbox::SetupFailure;
use trill_windows_sandbox::LOG_FILE_NAME;
use trill_windows_sandbox::SETUP_VERSION;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(trill_home) = std::env::var("CODEX_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&trill_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.trill_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.trill_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.trill_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.trill_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.trill_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.trill_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.trill_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.trill_home).join("sandbox_users.json");
