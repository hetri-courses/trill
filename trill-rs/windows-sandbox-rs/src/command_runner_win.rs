...
use anyhow::Context;
use anyhow::Result;
use trill_windows_sandbox::allow_null_device;
use trill_windows_sandbox::convert_string_sid_to_sid;
use trill_windows_sandbox::create_process_as_user;
use trill_windows_sandbox::create_readonly_token_with_cap_from;
use trill_windows_sandbox::create_workspace_write_token_with_cap_from;
use trill_windows_sandbox::get_current_token_for_restriction;
use trill_windows_sandbox::hide_current_user_profile_dir;
use trill_windows_sandbox::log_note;
use trill_windows_sandbox::parse_policy;
use trill_windows_sandbox::to_wide;
use trill_windows_sandbox::SandboxPolicy;
...
struct RunnerRequest {
    ...
    // Writable location for logs (sandbox user's .trill).
    trill_home: PathBuf,
    // Real user's CODEX_HOME for shared data (caps, config).
    real_trill_home: PathBuf,
    cap_sid: String,
    command: Vec<String>,
    cwd: PathBuf,
...
pub fn main() -> Result<()> {
    ...
    let log_dir = Some(req.trill_home.as_path());
    hide_current_user_profile_dir(req.trill_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_trill_home={}",
            req.cwd.display(),
            req.command,
            req.real_trill_home.display()
        ),
        Some(&req.trill_home),
    );
