...
pub fn cap_sid_file(trill_home: &Path) -> PathBuf {
    trill_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(trill_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(trill_home);
    if path.exists() {
