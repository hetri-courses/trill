...
def main() -> int:
    ...
    # 34. WS: policy tamper (.trill artifacts) denied
    codex_home = Path(os.environ["USERPROFILE"]) / ".trill"
    ...
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .trill\\policy.json"], WS_ROOT)
    add("WS: .trill cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .trill policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
