...
impl CloudRequirementsService {
    ...
    async fn fetch_with_timeout(&self) -> Option<ConfigRequirementsToml> {
        let _timer =
            trill_otel::start_global_timer("codex.cloud_requirements.fetch.duration_ms", &[]);
...
mod tests {
    ...
    fn write_auth_json(trill_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
        std::fs::write(trill_home.join("auth.json"), serde_json::to_string(&value)?)?;
        Ok(())
    }
