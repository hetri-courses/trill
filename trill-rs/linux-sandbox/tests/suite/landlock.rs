...
use trill_core::config::types::ShellEnvironmentPolicy;
use trill_core::error::CodexErr;
use trill_core::error::SandboxErr;
use trill_core::exec::ExecParams;
use trill_core::exec::process_exec_tool_call;
use trill_core::exec_env::create_env;
use trill_core::protocol::SandboxPolicy;
use trill_core::protocol_config_types::WindowsSandboxLevel;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> trill_core::exec::ExecToolCallOutput {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_trill-linux-sandbox");
    let trill_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_trill-linux-sandbox");
    let trill_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
