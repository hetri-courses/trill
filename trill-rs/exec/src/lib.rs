...
use trill_core::LMSTUDIO_OSS_PROVIDER_ID;
use trill_core::NewThread;
use trill_core::OLLAMA_CHAT_PROVIDER_ID;
use trill_core::OLLAMA_OSS_PROVIDER_ID;
...
use trill_core::git_info::get_git_repo_root;
...
use trill_protocol::approvals::ElicitationAction;
...
struct ThreadEventEnvelope {
    thread_id: trill_protocol::ThreadId,
    thread: Arc<trill_core::TrillThread>,
    event: Event,
}

pub async fn run_main(cli: Cli, trill_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("trill_exec".to_string()) {
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        trill_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, false)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
    ...
    let (initial_operation, prompt_summary) = match (command, prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = trill_core::review_prompts::user_facing_hint(&review_request.target);
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        match trill_core::RolloutRecorder::find_latest_thread_path(
            &config.trill_home,
            1,
            None,
            trill_core::ThreadSortKey::UpdatedAt,
            &[],
        ...
        if Uuid::parse_str(id_str).is_ok() {
            let path = find_thread_path_by_id_str(&config.trill_home, id_str).await?;
            Ok(path)
        } else {
            let path = find_thread_path_by_name_str(&config.trill_home, id_str).await?;
            Ok(path)
        }
    } else {
        Ok(None)
