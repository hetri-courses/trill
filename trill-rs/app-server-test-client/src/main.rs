...
use trill_app_server_protocol::InputItem;
...
use trill_app_server_protocol::SandboxPolicy;
...
#[derive(Parser)]
#[command(author = "Trill", version, about = "Bootstrap Codex app-server", long_about = None)]
struct Cli {
...
impl CodexClient {
    fn spawn(codex_bin: &str, config_overrides: &[String]) -> Result<Self> {
        ...
        let stdin = trill_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = trill_app_server
            .stdout
            .take()
            .context("codex app-server stdout unavailable")?;

        Ok(Self {
            child: trill_app_server,
            stdin: Some(stdin),
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "trill-toy-app-server".to_string(),
    ...
    fn remove_thread_listener(&mut self, subscription_id: Uuid) -> Result<()> {
        ...
        let request = ClientRequest::RemoveConversationListener {
            request_id: request_id.clone(),
            params: trill_app_server_protocol::RemoveConversationListenerParams { subscription_id },
        };

        self.send_request::<trill_app_server_protocol::RemoveConversationSubscriptionResponse>(
            request,
            request_id,
