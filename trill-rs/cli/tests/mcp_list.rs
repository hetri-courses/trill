...
fn codex_command(trill_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::new(trill_utils_cargo_bin::cargo_bin("codex")?);
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = codex_command(trill_home.path())?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = codex_command(trill_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(trill_home.path()).await?;
    ...
    let mut list_cmd = codex_command(trill_home.path())?;
    ...
    let mut list_json_cmd = codex_command(trill_home.path())?;
    ...
    let mut get_cmd = codex_command(trill_home.path())?;
    ...
    let mut get_json_cmd = codex_command(trill_home.path())?;
    get_json_cmd
