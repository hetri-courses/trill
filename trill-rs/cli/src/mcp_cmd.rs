...
use trill_core::config::load_global_mcp_servers;
use trill_core::config::types::McpServerConfig;
use trill_core::config::types::McpServerTransportConfig;
use trill_core::mcp::auth::McpOAuthLoginSupport;
use trill_core::mcp::auth::compute_auth_statuses;
use trill_core::mcp::auth::oauth_login_support;
use trill_core::protocol::McpAuthStatus;
use trill_rmcp_client::delete_oauth_tokens;
use trill_rmcp_client::perform_oauth_login;

/// Subcommands:
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.trill/config.toml`
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let trill_home = find_trill_home().context("failed to resolve CODEX_HOME")?;
    let mut servers = load_global_mcp_servers(&trill_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", trill_home.display()))?;
    ...
    ConfigEditsBuilder::new(&trill_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", trill_home.display()))?;
