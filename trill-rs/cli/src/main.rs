...
use trill_chatgpt::apply_command::ApplyCommand;
use trill_chatgpt::apply_command::run_apply_command;
use trill_cli::LandlockCommand;
use trill_cli::SeatbeltCommand;
use trill_cli::WindowsCommand;
use trill_cli::login::read_api_key_from_stdin;
use trill_cli::login::run_login_status;
use trill_cli::login::run_login_with_api_key;
use trill_cli::login::run_login_with_chatgpt;
use trill_cli::login::run_login_with_device_code;
use trill_cli::login::run_logout;
use trill_cloud_tasks::Cli as CloudTasksCli;
use trill_common::CliConfigOverrides;
use trill_exec::Cli as ExecCli;
use trill_exec::Command as ExecCommand;
use trill_exec::ReviewArgs;
use trill_execpolicy::ExecPolicyCheckCommand;
use trill_responses_api_proxy::Args as ResponsesApiProxyArgs;
use trill_tui::AppExitInfo;
use trill_tui::Cli as TuiCli;
use trill_tui::ExitReason;
use trill_tui::update_action::UpdateAction;
...
use trill_core::features::Stage;
use trill_core::features::is_known_feature_key;
use trill_core::terminal::TerminalName;
...
#[clap(
    ...
    // `trill-x86_64-unknown-linux-musl`, but the help output should always use
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    let mut lines = vec![format!(
        "{}",
        trill_core::protocol::FinalOutput::from(token_usage)
    )];

    if let Some(resume_cmd) =
        trill_core::util::resume_command(thread_name.as_deref(), conversation_id)
    {
...
fn stage_str(stage: trill_core::features::Stage) -> &'static str {
    use trill_core::features::Stage;
    match stage {
...
fn main() -> anyhow::Result<()> {
    arg0_dispatch_or_else(|trill_linux_sandbox_exe| async move {
        cli_main(trill_linux_sandbox_exe).await?;
        Ok(())
    })
}

async fn cli_main(trill_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    let MultitoolCli {
    ...
    match subcommand {
        None => {
            ...
            let exit_info = run_interactive_tui(interactive, trill_linux_sandbox_exe).await?;
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            trill_exec::run_main(exec_cli, trill_linux_sandbox_exe).await?;
        }
        ...
        Some(Subcommand::McpServer) => {
            trill_mcp_server::run_main(trill_linux_sandbox_exe, root_config_overrides).await?;
        }
        ...
        Some(Subcommand::AppServer(app_server_cli)) => match app_server_cli.subcommand {
            None => {
                trill_app_server::run_main(
                    trill_linux_sandbox_exe,
                    root_config_overrides,
                    trill_core::config_loader::LoaderOverrides::default(),
                    app_server_cli.analytics_default_enabled,
                )
                .await?;
            }
            Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                trill_app_server_protocol::generate_ts(
                    &gen_cli.out_dir,
                    gen_cli.prettier.as_deref(),
                )?;
            }
            Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                trill_app_server_protocol::generate_json(&gen_cli.out_dir)?;
            }
        },
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            trill_cloud_tasks::run_main(cloud_cli, trill_linux_sandbox_exe).await?;
        }
        Some(Subcommand::Sandbox(sandbox_args)) => match sandbox_args.cmd {
            SandboxCommand::Macos(mut seatbelt_cli) => {
                ...
                trill_cli::debug_sandbox::run_command_under_seatbelt(
                    seatbelt_cli,
                    trill_linux_sandbox_exe,
                )
                .await?;
            ...
            SandboxCommand::Linux(mut landlock_cli) => {
                ...
                trill_cli::debug_sandbox::run_command_under_landlock(
                    landlock_cli,
                    trill_linux_sandbox_exe,
                )
                .await?;
            ...
            SandboxCommand::Windows(mut windows_cli) => {
                ...
                trill_cli::debug_sandbox::run_command_under_windows(
                    windows_cli,
                    trill_linux_sandbox_exe,
                )
                .await?;
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            tokio::task::spawn_blocking(move || trill_responses_api_proxy::run_main(args))
                .await??;
        }
        ...
        Some(Subcommand::Features(FeaturesCli { sub })) => match sub {
            FeaturesSubcommand::List => {
                ...
                .await?;
                let mut rows = Vec::with_capacity(trill_core::features::FEATURES.len());
                let mut name_width = 0;
                let mut stage_width = 0;
                for def in trill_core::features::FEATURES.iter() {
                    let name = def.key;
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    maybe_print_under_development_feature_warning(&trill_home, interactive, feature);
    Ok(())
}
...
fn maybe_print_under_development_feature_warning(
    ...
    let Some(spec) = trill_core::features::FEATURES
        .iter()
    ...
    let config_path = trill_home.join(trill_core::config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_interactive_tui(
    ...
    trill_tui::run_main(interactive, trill_linux_sandbox_exe).await
}
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(trill_exec::Command::Resume(args)) = exec.command else {
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_matches!(
            ...
            Some(trill_common::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(trill_common::ApprovalModeCliArg::OnRequest)
        );
